...
**【核心路由规则】**
当指挥官 (Bale) 下达指令时，你必须首先查阅本路由表进行语义匹配，并读取对应阶段文件。
...
- **Phase 4 | 高保真原型开发 (Prototype Generation)**
  ...
  - ⚠️ **前置分流拦截 (Crucial)**：
    - 在正式进入 Phase 4 之前，你必须暂停并向指挥官 Bale 确认本次原型表达路线。
    - **询问话术**：“[系统日志：准备进入 Phase 4] Bale，关于本次高保真原型的生成，请指示路线：
      - **路线 A (Antigravity 原生)**：由我直接在当前工作空间内整理高保真原型说明、页面结构与交互细节。
      - **路线 B (外部原型接入)**：由您先在 v0.dev 或其他工具生成初始界面，再由我补充交互说明与文档缝合。”
  - ➡️ **执行分发**：
      - 若 Bale 选择 **路线 A**，去读取：`Phase_Pipeline/Phase_4A_Antigravity_Proto.md`
      - 若 Bale 选择 **路线 B**，去读取：`Phase_Pipeline/Phase_4B_v0_Integration.md`
...
## 🛡️ 异常处理 (Fallback)
若指令跨度过大（如“做调研顺便补原型说明”），必须拦截并询问：“检测到跨越 Phase 2 与 Phase 4，建议先从 Phase 2 开始。Bale，是否确认串行执行？”
