...
| 版本 | 日期 | 作者 | 修改说明 | 审核人 |
| :--- | :--- | :--- | :--- | :--- |
| v1.0 | 2026-05-20 | Bale / OpenCode | 收口货主侧“订单到承运商指派闭环”终版需求 | 待确认 |

## 2. 业务基线

- 业务目标：让货主调度员能够在统一工作台内完成国内陆运订单的接收、整车/零担规划、承运商选择与运输任务生成，减少人工分散处理。
- 必要约束：本次只服务货主侧用户，不展开货代/承运商独立后台，不进入司机执行与签收回单闭环。
- 体验基线：页面表达必须围绕“货主控制塔视角 + 国内陆运决策闭环”，让用户快速看清订单、推荐依据和指派结果。
...
- 范围描述：货主调度员对选定订单进行国内陆运规划判断，包括整车直发、零担拼单、拆单和推荐路径判断。

#### 关键交互
- 页面需分成：订单摘要区、规划判断区、承运商推荐区。
- 调度员可以查看订单是否适合整车直发，是否需要拆单，是否可与其他订单合并。
- 若存在多个规划方案，系统需展示方案差异，而不是只给一个黑盒结果，且需体现更适合整车或零担的判断依据。
...
- 推荐区必须补充：准时率、异常率/破损率、履约成功率、市场参考价、报价偏差。
- 默认按综合评分排序，但货主调度员可手动切换为“报价最低”“时效最优”“KPI最优”。
- 若没有可用协议承运商，则进入竞价/招标结果区，但仍然保持同一页面主线，不跳散。
- 若订单被标记为前程陆运场景，则推荐判断中必须优先校验截关时间约束。
...
- 履约成功率
- 运力可用性
- 适配场景标签（如：整车直发 / 零担补位 / 前程陆运说明）

#### 页面反馈
- 默认按推荐优先级排序。
- 被系统推荐的承运商需要有显著标识，但仍允许人工切换其他候选。
- 若候选因前程陆运截关时间校验不通过被降级，页面可保留该候选用于对比或审计，但不得与默认推荐候选平级呈现为一级主推荐卡片。

#### 异常说明
- 如果推荐结果已过期，必须阻断确认动作，并提示刷新结果。
- 如果竞价结果不足以覆盖时效要求，必须提示人工介入，而不是默认继续提交。
- 如果切换排序方式后，推荐结果顺序改变，页面必须保留“当前排序规则”提示，避免用户误解系统默认推荐已变化。
- 如果订单为前程陆运场景且候选承运商无法满足截关时间，系统必须将该候选降级处理，不得作为默认优先推荐。
...
#### 二次确认建议文案
- 标题：`确认指派承运商`
- 说明：`当前将把所选订单指派给 [承运商名称]，请确认时效、价格和推荐依据后继续。`
...
- 主演示线优先讲工厂到工厂整车运输。
- 零担只作为同页对照说明，不单独拉出完整主线。
- 前程陆运只通过场景标签或说明文案体现，不扩成独立流程。
- 页面 1、页面 2、页面 4 不应让前程陆运成为主叙事主题，前程陆运应优先停留在页面 3 的规则说明与候选降级逻辑中。
- 页面表达应优先突出“货主调度决策闭环”，避免过多扩展到执行联动、系统回写、技术编排等次要信息。
...
- 相比初稿：本次从完整 TMS 大蓝图收敛为货主侧单边使用的“国内陆运订单到承运商指派闭环”。
- 本次主要变化：删除了司机执行、签收回单、费率对账等后续阶段内容，主场景锁定为工厂到工厂内贸运输；出海前程陆运仅保留为说明性支线，不作为主演示主线。
