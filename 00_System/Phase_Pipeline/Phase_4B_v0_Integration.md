...
> 2. `/02_Agent_Roles/04_staff.md` (资深前端研发专家：负责代码剥离、状态绑定与 AntD 融合)
...
- **AntD 替换评估**：如果 v0 中的某些组件（如复杂的表格、分页、日期选择器）过于简陋，必须将其强制替换为 **Ant Design v5** 的标准组件，以保证企业级交互深度。保留 v0 优秀的卡片布局和微件。
