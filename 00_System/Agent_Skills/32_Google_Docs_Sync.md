...
> **[自动化触发器]**：当 `Project_Identity.json` 中的 `auto_save_as_gdoc` 为 `true` 时，Phase 3 结束即刻触发。
...
- **`Code`** -> 转为 Docs 【等宽字体 Courier New, 背景色 #F0F0F0】

## 2. 自动寻址与命名 (Auto-Naming)
- **路径解析**：
  1. 定位 `My Drive/Trans_Wise/01_PRD_Archive/`。
  2. 若当前 PRD 属于 v1.1.0，则自动创建/进入 `v1.1.0` 子文件夹。
- **命名规范**：`[TW_PRD]_[模块名]_v[版本号]_[日期]`

## 3. 附件处理 (Assets Injection)
- **Mermaid 转换**：由于 Docs 不支持 Mermaid 源码，AI 必须生成该流程图的文字描述版，并标注 `[需指挥官手动截图替换]`。
- **链接引用**：在文档页脚自动附上本地 `.md` 源码的相对路径，确保双向追溯。